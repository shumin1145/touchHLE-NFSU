...
const kAudioFilePropertyFileFormat: AudioFilePropertyID = fourcc(b"ffmt");
...
fn AudioFileGetPropertyInfo(
    ...
    if !is_writable.is_null() {
        env.mem.write(is_writable, 0);
        // TODO: probably not always correct
    }
    0 // success
}
    ...
    match in_property_id {
        ...
        kAudioFilePropertyFileFormat => {
            ...
            if bundle_id.starts_with("com.ea.mirrorsedge.bv")
                ...
                || bundle_id.starts_with("com.ea.nfsucbvzh")
            {
                log!("Applying game-specific hack for EA game: returning WAVE for kAudioFilePropertyFileFormat in AudioFileGetProperty()");
